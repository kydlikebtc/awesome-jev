...
| Most recent successful link-check date (dates vary by row) | 2026-09-26 |
| Rows with call-site text evidence recorded (not a CI pass count) | 1122 |
| Patterns covered | 18 of 18 |
| Chinese summaries hand-written | 197 of 1208 |
| Retired links | 2 |
<!-- shape:end -->
...
- `retry-control` (6 of 1208) — Most apparent matches are false positives: an HTTP client advertising "observable retries" is not a retry decision. The first real one was a semantic circuit breaker asking whether an HTTP 200 is a silent failure.
